...
class RSSMCell(tools.Module):
    def __init__(
        ...
        with tf.name_scope(self._var_scope):
            self._cell = tfkl.GRUCell(units=self._detstate_size) #每层使用相同的GRUcell来计算

    def _prior(self, prev_state, context):
        with tf.name_scope(self._var_scope):
            residual = context #自上向下的一个解码
            decode_context = self.get(
                "decoder_for_context",tfkl.Dense,self._state_size + self._detstate_size,activation=tf.nn.relu
            )(context)
            decode_context = residual*0.1 + decode_context

            inputs = tf.concat([prev_state["sample"], decode_context], -1)
            hl = self.get(  
    ...
    def _posterior(self, obs_inputs, prev_state, context):
        with tf.name_scope(self._var_scope):
            ...
            residual_mean = prior["mean"]
            residual_stddev = prior["stddev"]
            
            feature_mean = self.get(
                "posterior_mean_dense", tfkl.Dense, self._state_size, activation=None
            )(hl)
            feature_stddev = self.get(
                "posterior_stddev_dense",
                tfkl.Dense,
                self._state_size,
                activation=tf.nn.softplus,
            )(hl + 0.54)
            mean = residual_mean + feature_mean
            stddev = residual_stddev + feature_stddev#残差
            if self._mean_only:
                sample = mean
            else:
                sample = tfd.MultivariateNormalDiag(mean, stddev).sample() #stddev是标准差
        return {
            "mean": mean,
            "stddev": stddev,
            "sample": sample,
