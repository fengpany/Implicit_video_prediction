...
class Step:
    def __init__(self, session):
        ...
        self.add = self._step.assign_add(1)

    def increment(self):
        #return self._step.assign_add(1)
        return self.add
...
def read_configs(config_path, base_config_path=None, **kwargs):
    if base_config_path is not None:
        base_config = yaml.load(Path(base_config_path).read_text(),Loader=yaml.FullLoader)
        config = base_config.copy()
        config.update(yaml.load(Path(config_path).read_text(),Loader=yaml.FullLoader))
        assert (
...
def scan(cell, inputs, use_obs, initial):
    ...
    #最终，tf.scan 函数的输出是一个张量，表示整个序列的隐藏状态。这个张量的最后一个元素就是整个序列的最终隐藏状态。
    outputs = tf.scan(
