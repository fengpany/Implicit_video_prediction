...
from matrics.lpips_tensorflow_master import lpips_tf
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--logdir",
        default="/home/guohr/cwvae_latest/logs/cwvae_my_model_correct/mazes/mazes_cwvae_rssmcell_3l_f6_decsd0.1_enchl3_ences1000_edchnlmult1_ss20_ds200_es200_seq100_lr0.0001_bs50/model_495000",
        type=str,
        #required=True,
        help="path to dir containing model checkpoints (with config in the parent dir)",
    )
    parser.add_argument(
        "--num-examples", default=250, type=int, help="number of examples to eval on"
    )
    parser.add_argument(
        "--eval-seq-len",
        default=None,
    ...
    for i_ex in range(args.num_examples):
        try:
            ...
            #computing LPIPS:
            if(preds.shape[-1] == 1):
                distance_preds_input = np.tile(preds,[1,1,1,1,3])
                distance_gts_input = np.tile(gts[:,args.open_loop_ctx:],[1,1,1,1,3])
            else:
                distance_preds_input = preds
                distance_gts_input = gts[:,args.open_loop_ctx:]

            distance = session.run(distance_t, feed_dict={image0_ph:distance_gts_input, image1_ph:distance_preds_input})
            distance = np.mean(distance,axis=1)
            min_distance = np.min(distance)
            lpips_all.append(min_distance)
            ...
            if args.num_samples > 1 and save_npz:
    ...
    # Plotting.
    ssim_res_list = tools.plot_metrics(ssim_all, eval_logdir, "ssim")
    psnr_res_list = tools.plot_metrics(psnr_all, eval_logdir, "psnr")
    
    ssim_res = np.mean(ssim_res_list)
    psnr_res = np.mean(psnr_res_list)
    lpips_res = np.mean(lpips_all)
    print("ssim: ",ssim_res)
    print("psnr: ",psnr_res)
