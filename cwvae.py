...
class CWVAE:
    ...
    def hierarchical_unroll(
        ...
        for level in range(level_top, -1, -1):
            ...
            reset_state = tf.slice( #有的层可能会补点结点
                reset_state,
    ...
    def open_loop_unroll(self, inputs, ctx_len, actions=None, use_observations=None):
        ...
        (
            ...
            pre_inputs, actions=actions_pre, use_observations=use_observations #使用后验
        )
        _, _, post_priors, _ = self.hierarchical_unroll(#不使用后验
            post_inputs,
    ...
    def compute_losses(
        ...
        return self.loss,self._kl_term

def build_model(cfg, open_loop=True,Training = False):
    ...
    decoder = cnns.Decoder(cfg.channels, channels_mult=cfg.channels_mult,Training = Training)
    ...
    loss,kl_loss = model.compute_losses(
        obs,
        obs_decoded,
        priors,
        posteriors,
    ...
    if open_loop:
        posteriors_recon, priors_onestep, priors_multistep = model.open_loop_unroll( #pre_posteriors, pre_priors, post_priors
