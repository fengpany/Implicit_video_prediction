...
import tools
from layers_myadd import ResidualEncoderCell,ResidualDecoderCell
import os
...
class Encoder(tools.Module):
    ...
    def __init__(
        ...
        self.residual_cell = list([])
        self.residual_cell.append(ResidualEncoderCell(name = "resdual_cell_encoder_0"))
    ...
    def __call__(self, obs):
        ...
        with tf.name_scope(self._var_scope):
            ...
            hidden = tf.reshape(obs, tf.concat([[-1], tf.shape(obs)[2:]], -1)) #(BxT,64,64,3)

            filters = 32 
            hidden = self.get( #(31,31,32)
                "h1_conv", tfkl.Conv2D, self._channels_mult * filters, 4, **self._kwargs
            )(hidden)
            hidden = self.residual_cell[0](hidden,training = self.Training)

            hidden = self.get( #(14,14,64)
                "h2_conv",
                tfkl.Conv2D,
            ...
            for level in range(1, self._levels):
                ...
                residual_feature = tf.add(residual*0.1,hidden)
                residual = residual_feature
                layer = residual_feature
...
class Decoder(tools.Module):
    ...
    def __init__(self, out_channels, channels_mult=1, Training = False,var_scope="decoder_conv"):
        """
        Arguments:
            out_channels : int
        ...
        self._var_scope = var_scope
        self.residual_cell = list([])
        self.residual_cell.append(ResidualDecoderCell(name = "resdual_cell_decoder_0"))
        self.Training = Training
